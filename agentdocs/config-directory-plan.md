...
- `opencode-reviewer.toml` → базовые поля (`project_dir`, `[git]`, `[opencode]`, `[gitlab]`, и т.д.).
...
- поддержать загрузку TOML как из `--config`, так и из `<configDir>/opencode-reviewer.toml`;
