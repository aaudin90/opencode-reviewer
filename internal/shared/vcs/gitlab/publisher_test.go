...
func TestPublish_NoFindings(t *testing.T) {
	...
	if versionCount.Load() != 1 {
		t.Errorf("versions = %d, want 1", versionCount.Load())
	}
}
	...
	discussions := []Discussion{
		{
			...
			Notes:          []Note{{ID: 999, System: false, Resolvable: false, Resolved: false, Author: Author{ID: 99}}},
		},
	}
...
func TestPublish_DiffRefsFails(t *testing.T) {
	...
	err := pub.Publish(context.Background(), review, inline, "feature", "main")
	if err == nil {
		t.Fatal("expected error when diff refs fail")
	}
	if noteCount.Load() != 0 {
		t.Errorf("notes = %d, want 0", noteCount.Load())
	}
}
