...
func (c *Client) GetMergeRequest(ctx context.Context, mrIID int) (*MergeRequestInfo, error) {
	path := fmt.Sprintf("/projects/%d/merge_requests/%d", c.cfg.ProjectID, mrIID)
...
func (c *Client) ListDiscussions(ctx context.Context, mrIID int) ([]Discussion, error) {
	...
	for page := 1; ; page++ {
		...
		body, err := c.doGet(ctx, path, q)
		if err != nil {
			return result, fmt.Errorf("list discussions: %w", err)
		}
		var pageItems []Discussion
		if err := json.Unmarshal(body, &pageItems); err != nil {
			return result, fmt.Errorf("decode discussions: %w", err)
		}
...
func (c *Client) ReplyToDiscussion(ctx context.Context, mrIID int, discussionID, body string) error {
	path := fmt.Sprintf("/projects/%d/merge_requests/%d/discussions/%s/notes", c.cfg.ProjectID, mrIID, url.PathEscape(discussionID))
	if err := c.doPost(ctx, path, map[string]string{"body": body}); err != nil {
		return fmt.Errorf("reply to discussion: %w", err)
	}
	return nil
}

func (c *Client) SetDiscussionResolved(ctx context.Context, mrIID int, discussionID string, resolved bool) error {
	path := fmt.Sprintf("/projects/%d/merge_requests/%d/discussions/%s", c.cfg.ProjectID, mrIID, url.PathEscape(discussionID))
	if err := c.doPut(ctx, path, map[string]bool{"resolved": resolved}); err != nil {
		return fmt.Errorf("set discussion resolved: %w", err)
	}
	return nil
}
...
func (c *Client) doPut(ctx context.Context, path string, payload any) error {
