...
func TestCheckoutRemoteResetsBehindLocalBranch(t *testing.T) {
	client, workDir := setupRepo(t)
	tmp := filepath.Dir(workDir)
	...
	if err := os.WriteFile(filepath.Join(workDir, "feature.txt"), []byte("one\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	run(workDir, "add", "feature.txt")
	run(workDir, "commit", "-m", "feature one")
	run(workDir, "push", "origin", "feature")

	run(tmp, "clone", filepath.Join(tmp, "remote.git"), "other")
	run(otherDir, "config", "user.email", "test@test.com")
	run(otherDir, "config", "user.name", "Test")
	run(otherDir, "checkout", "feature")
	if err := os.WriteFile(filepath.Join(otherDir, "feature.txt"), []byte("two\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	run(otherDir, "commit", "-am", "feature two")
	run(otherDir, "push", "origin", "feature")

	if err := client.Fetch(); err != nil {
		t.Fatalf("Fetch() returned error: %v", err)
	}
	if err := client.CheckoutRemote("feature"); err != nil {
		t.Fatalf("CheckoutRemote() returned error: %v", err)
	}

	local := revParse(t, workDir, "feature")
	remote := revParse(t, workDir, "origin/feature")
	if local != remote {
		t.Fatalf("local feature = %s, want origin/feature %s", local, remote)
	}
}
