...
If the input includes a <source_review_prompt>...</source_review_prompt> block, treat it as the prompt/message that produced the original reviewer comment. Use that prompt as additional context for the intended review rules and scope when deciding whether the original reviewer comment is actually fixed or still valid.
For resolved AI findings, check the human replies, current code, and source review prompt context before acting. If the code was fixed, use reply or resolve with body to say it is fixed and do not unresolve. If a human explains the finding is false positive or asks to close it, use reply or resolve with body to confirm the closure unless the current code proves the finding is still valid. If reopening a discussion, use unresolve with body to explain why. Use noop for low confidence or when a human is needed.`
