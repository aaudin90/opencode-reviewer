...
func TestLoaderLoadsConfigDirAfterCheckout(t *testing.T) {
	...
	if err := pipeline.PrepareRepository(client, "feature"); err != nil {
		t.Fatalf("PrepareRepository: %v", err)
	}

	resources := loadRuntime(t, workDir)
	if len(resources.Messages) != 1 || resources.Messages[0] != "branch message" {
...
func TestLoaderUsesConfigDirVersionFromCheckedOutBranch(t *testing.T) {
	...
	resources := loadRuntime(t, workDir)
	if len(resources.Messages) != 1 || resources.Messages[0] != "feature message" {
...
func TestLoaderUsesDeprecatedMessageEnvWhenConfigDirInactive(t *testing.T) {
	...
	if err := pipeline.PrepareRepository(client, "main"); err != nil {
		t.Fatalf("PrepareRepository: %v", err)
	}

	resources := loadRuntime(t, workDir)
	if len(resources.Messages) != 1 || resources.Messages[0] != "env message" {
...
func TestLoaderIgnoresDeprecatedMessageEnvWhenConfigDirActive(t *testing.T) {
	...
	resources := loadRuntime(t, workDir)
	if len(resources.Messages) != 1 || resources.Messages[0] != "config-dir message" {
